...
<header><a class=\"brand\" href=\"/\" style=\"color:inherit;text-decoration:none\">러니웨어<span style=\"color:#66726a;font-weight:600\">: 어디서든 러닝 코스 짜기!</span></a><nav><a href=\"/\">서비스 소개</a><a href=\"/animals\">동물 GPS 아트</a></nav></header>{body}
...
<h2>주요 출처</h2><ul><li><a href="https://www.openstreetmap.org/copyright">© OpenStreetMap contributors · ODbL 1.0</a></li><li><a href="https://data.seoul.go.kr/dataList/OA-22241/F/1/datasetView.do">서울시 경사도 OA-22241</a></li><li><a href="https://data.seoul.go.kr/dataList/OA-22205/F/1/datasetView.do">서울시 가로등 OA-22205</a></li><li><a href="https://data.seoul.go.kr/dataList/OA-22356/F/1/datasetView.do">서울시 보행자 신호등 OA-22356</a></li><li><a href="https://data.seoul.go.kr/dataList/OA-22586/S/1/datasetView.do">서울시 공중화장실 OA-22586</a></li><li><a href="https://www.data.go.kr/data/15044231/fileData.do">서울교통공사 역주소</a> · <a href="https://www.data.go.kr/data/15099316/fileData.do">1–8호선 역 좌표</a></li><li><a href="https://registry.opendata.aws/terrain-tiles/">Mapzen Terrain Tiles</a> · <a href="https://doi.org/10.5067/MEaSUREs/SRTM/SRTMGL1.003">SRTMGL1 v3</a> · SRTM data courtesy of the U.S. Geological Survey</li></ul>
<h2>가공 및 제공</h2><p>원천 데이터는 좌표 변환·필터링·결합·점수화되었으며 공식 실시간 자료가 아닙니다. OSM 파생 DB와 재생성 코드는 <a href="https://github.com/Querry-AI/Runnywhere-PlayMCP">공개 소스 저장소</a>의 <code>data/</code>, <code>etl/</code>, <code>scripts/build_animal_presets.py</code>, <code>scripts/build_standard_presets.py</code>에서 ODbL 조건으로 제공됩니다. 전체 고지는 저장소의 <code>DATA_LICENSES.md</code>를 참고하세요.</p><p class="muted">출처 표시는 각 기관의 후원이나 인증을 의미하지 않습니다.</p></main>"""
    else:
...
/* Sharing needs somewhere to answer. The label-swap it used to rely on is wiped
   the moment the silhouette finishes loading and the card re-renders, and on a
   phone the detail sheet can sit below the fold entirely. */
.page-toast{position:fixed;z-index:80;left:16px;right:16px;bottom:calc(20px + env(safe-area-inset-bottom));
 margin:0 auto;max-width:420px;display:flex;align-items:center;gap:10px;min-height:48px;padding:12px 15px;
 border-radius:14px;background:rgba(19,32,26,.96);color:#fff;font-size:14px;font-weight:700;line-height:1.45;
 box-shadow:0 10px 30px rgba(10,28,19,.34);word-break:keep-all;cursor:pointer;
...
.page-toast span{flex:1;min-width:0}
@media(prefers-reduced-motion:reduce){.page-toast{transition:none}}
</style>
...
<div id="pageToast" class="page-toast" role="status" aria-live="polite" data-tone="ok" hidden><span id="pageToastText"></span></div>
...
const pageToast=document.getElementById('pageToast');const pageToastText=document.getElementById('pageToastText');let pageToastTimer=null;
function hidePageToast(){if(!pageToast)return;clearTimeout(pageToastTimer);pageToastTimer=null;delete pageToast.dataset.open;pageToast.hidden=true}
function showPageToast(text,tone){if(!pageToast)return;clearTimeout(pageToastTimer);pageToast.dataset.tone=tone||'ok';pageToast.hidden=false;pageToastText.textContent=text;void pageToast.offsetWidth;pageToast.dataset.open='';pageToastTimer=setTimeout(hidePageToast,2600)}
if(pageToast)pageToast.addEventListener('click',hidePageToast);
// The only copy path left when the async clipboard is missing or refused. The
// old fallback was window.prompt, which several mobile browsers suppress
// outright -- there the press did nothing at all and said nothing either.
function copyBySelection(text){const field=document.createElement('textarea');field.value=text;field.setAttribute('readonly','');field.style.cssText='position:fixed;top:0;left:0;width:1px;height:1px;opacity:0';document.body.appendChild(field);field.focus();field.select();if(field.setSelectionRange)field.setSelectionRange(0,text.length);let copied=false;try{copied=document.execCommand('copy')}catch(err){copied=false}field.remove();return copied}
// Feedback goes to the toast, not to the button's own label: the card is
// re-rendered as soon as the silhouette loads, which wiped the label mid-read.
function shareCourse(course){const url=baseUrl+'/c/'+course.cid;const done=()=>showPageToast('링크가 복사되었습니다','ok');const fallback=()=>{if(copyBySelection(url))done();else showPageToast('링크를 복사하지 못했어요. 주소창의 주소를 복사해 주세요.','error')};if(navigator.clipboard&&navigator.clipboard.writeText)navigator.clipboard.writeText(url).then(done).catch(fallback);else fallback()}
