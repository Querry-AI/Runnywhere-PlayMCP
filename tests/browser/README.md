...
| 지우개 | 훑은 구간이 표시되고, 지우기가 `reroute`를 보내고, 돌아온 거리가 화면에 반영된다 |
| 지우개 범위 | 반대편으로 손가락이 튀어도 선택이 코스 전체로 번지지 않는다 |
| 경유점 | 탭 한 번이 `via` 요청 하나를 만들고, 짚은 좌표와 구간이 함께 실린다 |
| 이름 짓기 | 저장이 이름 시트를 열고, 빈 칸으로 저장하면 `name:""`이 간다 |
...
`34/34 passed`가 나와야 하고, 실패가 있으면 종료 코드가 0이 아니다. Playwright(`npm i -g playwright`)가 필요하다.
