...
- 특정 출발지에서 요청한 코스가 없고 인근 대안만 있으면 `start_change_confirmation_required`로 질문만 반환한다. 위젯·코스 ID·지도 링크는 보내지 않는다. 사용자가 가까운 출발지의 같은 모양 또는 원래 출발지의 일반 코스를 선택한 **다음 턴**에 `confirmation_options`의 인자로 통합 툴을 호출한다. 두 선택지에 모호한 “네”만 답하면 다시 구분해서 묻는다.
- `allow_nearby_start`는 기본 `false`다. 가까운 출발지 선택에 명시적으로 동의한 경우에만 `true`로 호출하며 원래 거리·지형·야간·시설 조건은 유지한다. 동의 전에는 정확한 출발지 카드만 제공하고, 다른 출발지 카드로 빈자리를 채우지 않는다(역 출구 허용 범위 150m 미만). 구/공원 목적지 카탈로그 계약은 유지한다.
- 일반 코스의 첫 카드는 해당 출발지의 일반 코스다. 동의한 경우에만 2km 안의 조건 충족 동물 프리셋을 추가하고, 그 외에는 같은 출발지 변형만 제공한다. 첫 코스가 없으면 다른 출발점/종류로 대체하지 않는다.
